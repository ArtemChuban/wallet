...
**Domain:** Local single-user personal finance / net-worth tracking (Docker + SQLite, balance snapshots, dated FX, charts)
**Researched:** 2026-09-02
**Confidence:** HIGH for package versions (npm registry verified same day); MEDIUM for framework choice (ecosystem patterns + project constraints, not a single canonical “standard”)

## Recommended Stack

**Prescriptive one-liner:** Node 24 LTS + Hono API + Vite/React SPA + Drizzle + better-sqlite3 + Recharts + decimal.js, one Docker container, SQLite on a host bind-mount.
...
npm install hono @hono/node-server @hono/zod-validator \
  drizzle-orm better-sqlite3 zod decimal.js dotenv
...
npm install -D typescript vite @vitejs/plugin-react \
  ...
  tailwindcss @tailwindcss/vite vitest tsx
```

Pin exact versions in the lockfile; table versions above are **npm latest as of 2026-09-02**.
...
**If staying strictly SPA + API (recommended for Wallet):**
...
**If team strongly prefers one meta-framework:**
...
---
*Stack research for: local Docker + SQLite personal finance / net-worth web app*
*Researched: 2026-09-02*
